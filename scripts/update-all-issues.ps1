...
$h = @{ Authorization = "token $Token"; "User-Agent" = "kiro"; Accept = "application/vnd.github.v3+json" }
$repo = "Hahfyeex/Stellar-PolyMarket"

function Patch($num, $body) {
    $p = @{ body = $body } | ConvertTo-Json -Depth 3
    try {
        $r = Invoke-RestMethod "https://api.github.com/repos/$repo/issues/$num" -Method PATCH -Headers $h -Body $p -ContentType "application/json"
        Write-Host "OK #$num: $($r.title)"
    } catch { Write-Host "FAIL #$num: $_" }
    Start-Sleep -Seconds 2
}

$checkout = "`n`n## PR and Checkout`n``````bash`ngit checkout -b feat/issue-NUMBER-short-title`n# make your changes`ngit add .`ngit commit -m 'feat: description of change'`ngit push origin feat/issue-NUMBER-short-title`n``````\nThen open a PR against ``main`` and link this issue in the PR description using ``Closes #NUMBER``."
...
2. Use a global Context or Redux slice to manage ``isOpen``, ``bets[]``, and ``addBet``/``removeBet`` actions
3. Each queued bet item should show: market title, selected outcome, amount, and a remove button
...
## Guidelines
- Slide-up drawer animating from bottom on mobile, side panel on desktop
...
3. Map percentage to CSS opacity: 0-20% = 0.1 opacity, 20-50% = 0.3, 50-80% = 0.6, 80-100% = 0.9
4. Render a ``div`` overlay behind each order book row using ``position: absolute``
5. YES-side rows use ``background: rgba(59, 130, 246, opacity)`` (blue), NO-side use ``rgba(249, 115, 22, opacity)`` (orange)
...
2. Create a ``getBadgeTier(marketsCount, accuracyPct)`` utility function with the tier thresholds
3. Build a ``<ReputationBadge tier={tier} size={size} />`` component that renders the correct SVG
4. Add CSS ``box-shadow`` glow on ``:hover`` — each tier has a unique glow color (bronze: amber, silver: gray, gold: yellow, diamond: cyan)
5. Fetch user stats from ``GET /api/users/:wallet/stats`` and compute tier on the frontend
6. Render badge on profile page header and as a small icon next to wallet address in leaderboard rows
...
- Badges appear on user profile and leaderboard
...
## Figma Reference
Figma Link: _Add Figma frame link here before starting work_
> Contributor: attach the Figma frame link before submitting your PR.
...
## Implementation Guide
1. Create a ``SocialTicker`` component that renders a horizontal scrolling strip
...
5. Use Framer Motion ``AnimatePresence`` + ``motion.div`` with ``initial={{ x: 100, opacity: 0 }}`` and ``exit={{ x: -100, opacity: 0 }}``
...
6. Persist ``onboardingComplete: true`` to localStorage on final step or skip
7. Check localStorage on app mount — skip wizard if already complete
8. Progress stepper: render 4 dots, fill active and completed ones
...
Write-Host "=== UIUX 116-120 updated ==="
...
2. Input: a range slider (0 to max wallet balance) and a number input field, kept in sync
...
6. Update all values on every slider/input change using ``useEffect`` with debounce (200ms)
...
3. If change exceeds ``VOLATILITY_THRESHOLD`` (default 5), add a CSS class ``pulse-green`` or ``pulse-red``
...
2. Add SVG illustrations per category in ``/public/categories/``: sports.svg, crypto.svg, finance.svg, politics.svg, weather.svg
...
5. Hover effect: ``transform: translateY(-4px)`` + ``box-shadow`` increase on ``:hover``
...
Write-Host "=== UIUX 121-125 updated ==="

# ── FRONTEND ISSUES (126-140) ──────────────────────────────────────────────
...
3. Common batch flows to support: ``[placeBet, addTrustline]``, ``[placeBet, payFee]``
...
## Implementation Guide
1. Create ``useIPFSMetadata(cid: string)`` hook using Pinata SDK ``pinata.gateways.get(cid)``
2. Wrap fetch in React Query: ``useQuery(['ipfs', cid], () => fetchMetadata(cid), { staleTime: Infinity })``
...
4. Build a trustline transaction: ``new TransactionBuilder(account).addOperation(Operation.changeTrust({ asset }))``
...
3. Convert all page-level imports to ``next/dynamic``: ``const MarketDetail = dynamic(() => import('./MarketDetail'))``
4. In ``recharts``, import only used components: ``import { BarChart, Bar } from 'recharts'`` not the full package
5. In ``stellar-sdk``, use tree-shaking: ``import { TransactionBuilder } from '@stellar/stellar-sdk'``
...
3. Create an error code map in ``/constants/contractErrors.ts``: ``{ 'INSUFFICIENT_BALANCE': 'You do not have enough XLM...', 'MARKET_CLOSED': 'This market has ended...' }``
...
6. Alternatively use ``react-pull-to-refresh`` library if custom implementation is too complex
...
4. Replace all ``isLoading ? <Spinner /> : <Component />`` patterns with ``isLoading ? <ComponentSkeleton /> : <Component />``
5. Use ``min-height`` on containers to prevent CLS when content loads in
6. Shimmer CSS: ``background: linear-gradient(90deg, #1f2937 25%, #374151 50%, #1f2937 75%)`` animated with ``background-size: 200%``
...
## Implementation Guide
1. Create a reusable ``<CopyButton value={fullString} />`` component
2. Display abbreviated format: ``value.slice(0,6) + '...' + value.slice(-4)``
3. On click, call ``navigator.clipboard.writeText(value)``
4. Show a ``Copied!`` tooltip using a ``useState`` flag, auto-reset after 2000ms with ``setTimeout``
...
Write-Host "=== FE 136-140 updated ==="

# ── BACKEND ISSUES (141-155) ──────────────────────────────────────────────
...
3. ``place_bet``: require bettor auth, validate market is open and not expired, call ``token::Client::transfer`` to lock funds, record bet in Map
4. ``resolve_market``: require admin auth, validate winning_outcome index, set ``market.resolved = true``
5. ``distribute_rewards``: iterate bets, calculate each winner's share as ``(bet_amount * total_pool * 97/100) / winning_stake``, transfer payout
...
4. Retry logic: wrap oracle call in a loop with ``await sleep(2^attempt * 1000)`` up to 3 attempts
...
6. Write example queries in the README: get all bets for a market, get user win rate, get top markets by volume
...
2. ``open_dispute(market_id)``: callable by any token holder within 24h of resolution, sets ``dispute.active = true``
3. ``cast_vote(market_id, support)``: require voter auth, fetch STELLA token balance from token contract, add weighted vote
...
## Implementation Guide
1. Add ``DataKey::CreationFee`` and ``DataKey::FeeDestination`` (burn address or DAO treasury) to contract storage
...
## Implementation Guide
1. Create a ``/metrics`` endpoint using ``prom-client`` library
...
4. Alert rule in Prometheus: ``(tvl_total_xlm - tvl_total_xlm offset 5m) / tvl_total_xlm offset 5m < -0.2``
...
## Implementation Guide
1. Write a Go service with a main loop: ``for ledger := startLedger; ; ledger++``
...
4. Use a goroutine pool (``sync.WaitGroup`` + buffered channel) to process 10 ledgers concurrently
5. Store archived events in PostgreSQL with columns: ``ledger_seq``, ``tx_hash``, ``event_type``, ``data``, ``timestamp``
6. Catch-up mode: accept a ``--start-ledger`` CLI flag to process historical ledgers from a given point
...
2. Serialize the log entry to JSON and upload to IPFS via Pinata: ``pinata.pinJSONToIPFS(logEntry)``
...
## Implementation Guide
1. Create a ``validateMarket(metadata)`` function with 4 checks:
   - No duplicate: query DB for existing market with same question (case-insensitive)
   - Valid end date: ``endDate > Date.now() + 3600000`` (at least 1 hour from now)
   - Description length: ``description.length >= 50``
   - Outcome count: ``outcomes.length >= 2 && outcomes.length <= 5``
...
7. Add a CI step: ``bzt stress-test.yml --report`` that fails if p95 latency exceeds 2s or error rate exceeds 1%
...
Write-Host "=== BE 151-155 updated ==="
Write-Host "ALL 40 ISSUES FULLY UPDATED"
